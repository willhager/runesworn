...
func _ready() -> void :
	var enemyDict = EncounterData.get_encounter_by_name(1, "Goblin")
...
func clear() -> void :
	...
	for i in range(0, 3) :
		var eNode = get_node(eDieSpritePath + str(i) + eDieSpritePath2 + str(i))
		if GameState.EDice[i].get("freeze") == true :
